...
public class MovimientoManager {
    ...
    protected Movimiento readByDNI(String dni) {
        Session session = sessionFactory.openSession();

        Movimiento movimiento = session.get(Movimiento.class, dni);
    ...
    public List<Movimiento> buscarTodas() {
        ...
        Query query = session.createNativeQuery("SELECT * FROM persona", Movimiento.class);
     ...
     * @return
     */
    public List<Movimiento> buscarPor(String nombre) {
        ...
        //como nombre: "' or '1'='1"
        Query query = session.createNativeQuery("SELECT * FROM persona where nombre = '"+nombre+"'", Movimiento.class);
